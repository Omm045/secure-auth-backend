...
def test_register_and_change_reject_breached(monkeypatch):
    ...
    with TestClient(app) as client:
        assert client.post("/auth/register", json={"email": "breach-flow@example.com", "password": "abcdefgh"}).status_code == 201
        ...
        assert client.post("/auth/change-password", headers={"X-CSRF-Token": csrf},
                           json={"current_password": "abcdefgh", "new_password": "newpass1"}).status_code == 422
...
def test_reset_rejects_breached(monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert client.post("/password/reset", headers={"X-CSRF-Token": csrf},
                           json={"token": "invalid", "new_password": "abcdefgh"}).status_code == 422
