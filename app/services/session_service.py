...
def current_user(request:Request):
    ...
    with transaction() as c:
        row=c.execute("SELECT u.* FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token_hash=? AND s.revoked=0 AND s.expires_at>?",(token_hash(raw),datetime.now(timezone.utc).isoformat())).fetchone()
...
def revoke(request):
    raw=request.cookies.get("session") or request.headers.get("Authorization","").removeprefix("Bearer ").strip()
    if raw:
        with transaction() as c:c.execute("UPDATE sessions SET revoked=1 WHERE token_hash=?",(token_hash(raw),))
