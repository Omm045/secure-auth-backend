...
def reject_password(password, user_id=None):
    ...
    if is_password_compromised(password) or breach_checker.is_breached(password):
...
def set_session(response,raw,exp): response.set_cookie("session",raw,httponly=True,secure=settings.session_cookie_secure,samesite="lax",expires=exp)
@router.post("/register",status_code=201)
def register(data:Credentials,request:Request,response:Response):
    enforce_rate_limit(request,settings.rate_limit_per_minute); reject_password(data.password)
    with transaction() as c:
        try:c.execute("INSERT INTO users(email,password_hash,created_at) VALUES(?,?,?)",(data.email.lower(),hash_password(data.password),datetime.now(timezone.utc).isoformat())); uid=c.execute("SELECT last_insert_rowid()").fetchone()[0]
...
def logout(request:Request,response:Response):
    verify_csrf(request); revoke(request); response.delete_cookie("session"); return {"message":"Logged out"}
@router.get("/me")
...
def change(data:PasswordChange,request:Request,response:Response,user=Depends(current_user)):
    ...
    response.delete_cookie("session"); audit(user["id"],"change_password"); return {"message":"Password changed"}
